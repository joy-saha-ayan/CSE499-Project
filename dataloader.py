...
class MultimodalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if Modality(self.modality) in [Modality.IMAGE, Modality.TEXT_IMAGE, \
                                       Modality.TEXT_IMAGE_DIALOGUE]:
            image_path = os.path.join(IMAGES_DIR, str(item_id).zfill(4) + IMAGE_EXTENSION)
      ...
      def _preprocess_df(self, df):
        def image_exists(row):
            ...
            image_path = os.path.join(IMAGES_DIR, str(row['id']).zfill(4) + IMAGE_EXTENSION)
        ...
        df = df.drop(
            columns=['created_utc', 'domain', 'hasImage', 'image_url'],
            errors='ignore'
        )
